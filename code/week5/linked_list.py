"""
Use your LinkedList to make the Stack and Queue below.

I've provided the skeleton, you have to fill in the details

You can use the slides and ask me questions.
...
class Queue:
    ...
    Functions:
        insert: add an value to the end of the queue
        pop: remove the item at the top of the queue
        search: return True if the value is in the queue
    """
    def __init__(self):
        self.root = None
...
def test():
    myqueue = Queue()
    myqueue.insert(5)
    myqueue.insert("potato")
    myqueue.insert("bunnies")
    assert myqueue.search("potato") == True
    myqueue.pop()
    assert myqueue.search(5) == False


    mystack = Stack()
    mystack.insert(5)
    mystack.insert("potato")
    mystack.insert("bunnies")
    mystack.pop()
    assert mystack.search("bunnies") == False
